...
## 2026-09-26: tstokenlib 3.0.0, released from main again

**Why.** tstokenlib found and fixed deviation D1: a verifier squeezed its query indices from the transcript state before the grind, so the grind bought no soundness (88 bits conjectured / 44 proven for the system, 102 / 58 with the fix; tstokenlib `docs/SECURITY_CLAIM.md`). The fix ships only as tstokenlib 3.0.0. Proofs do not verify across the two versions, and a pool's PP1 carries its verifier forward round after round, so a pool created before 3.0.0 cannot move to it. The testnet pool created with 0.1.x is abandoned with its funds, and a new one is created with 0.2.0.

**Releasing from main.** Since v0.1.1 every release came from `release/0.1`, pinned to tstokenlib 2.0.x, which has no build hook: `dart compile exe` refuses a package with a hook, and on macOS `dart test` failed past a path of about 87 characters because the prebuilt kernels library had 56 bytes of header room for the absolute install name Dart writes into it. 3.0.0 has the hook, so both were fixed rather than avoided:
- tstokenlib links the Apple libraries with `-headerpad_max_install_names`, and its kernels' source hash now covers each target's features and flags, so the prebuilt libraries were rebuilt (`stark-kernels-c4f6a70ae9756d97`; the macOS arm64 library has 7,192 bytes of room).
- The release builds with `dart build cli`, as cloak-cli does. The version goes in through `lib/src/build_version.dart`, written for the build and put back, since `dart build cli` takes no `--define`. The kernels come from the hook, in the bundle's `lib/`; the workflow no longer builds them with cargo.

**Checked on 3.0.0.** The suite from this repository's 93-character path, `cli_test` included (196 passed, 3 skipped); the localnet end to end (a pool created, deposits and 1,100 messages through ricochet, rounds mined); the macOS package's `check` from `/`; and `tool/deb_e2e.sh` on arm64, every check from install to purge, which also needed two harness fixes (create's funding line, and `dart run`'s hook progress printed without a newline before the ricochet line). `release/0.1` is retired: 0.1.8 is its last release.
